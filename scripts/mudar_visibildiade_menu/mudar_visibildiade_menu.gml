...
<<<<<<< HEAD
    ...
    if (estado_visivel) {
        ...
        // Ativa a layer visual - sprites estáticos
        instance_activate_layer(id_layer_ui);
        layer_set_visible(id_layer_ui, true);
        
        // Traz os botões de volta na tela
        for (var i = 0; i < array_length(objs_interativos); i++) {
            var obj = objs_interativos[i];
            
           //Ativa o objto para exucutar o código abaixo
            instance_activate_object(obj);
            
            // Agora é executado o código abaixo para garantir a visibilidade
        ...
        // Esconde a layer visual
        layer_set_visible(id_layer_ui, false);
        instance_deactivate_layer(id_layer_ui);
        
        // Apaga e congela os botões
        for (var i = 0; i < array_length(objs_interativos); i++) {
            var obj = objs_interativos[i];
            
            //  O código abaixo torna invisível os objetos enquanto eles ainda estão ativos
            if (instance_exists(obj)) {
                with (obj) {
                    visible = false;
                    // Reseta o touch_id para evitar que o botão trave ao pausar
            ...
            // desativa completamente para economizar CPU
